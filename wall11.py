...
class wall(object):
    ...
    def create_geometry(self):
        ...
        #门 
        window = self.shape_cuboid(self.WLength_d,self.Width,self.WHeight+self.oWmobilePosition,sX=self.Length-self.WLength_d-self.WmobilePosition,sZ=self.oWmobilePosition_d)
        err,rectangle = AllplanGeo.MakeSubtraction(rectangle,window)
    ...
    def create_reinforcement(self):
        ...
        if  self.VertSteelVisual_q:
             ...
             reinforcement +=  self.create_vertical_steel_d()
        #水平钢筋
        if self.HoriSteelVisual:
             reinforcement +=  self.create_horizontal_steelr()
             reinforcement += self.create_horizontal_steelt_d()
        #拉筋
    ...
    def create_horizontal_steelr(self): 
        ...
        for i in range(lines_1):
                ...
                steel_list += self.create_horizontal_steelt(self.Length-self.WLength_d-self.WmobilePosition*2-2*self.HoriFrontCover,3,point_1,point_2,i,self.DegreesHook_HS2,self.HoriExtend,0)  #第二个参数hook_side:1 左延伸锚 2 右延伸锚 3左右延伸锚 0不延伸左右锚 4  5 
        steel_list +=self.create_horizontal_steel_modify(self.Length-self.WLength_d-self.WmobilePosition*2-2*self.HoriFrontCover,3,self.DegreesHook_HS2,self.HoriExtend,0,self.HoriFrontCover+self.WmobilePosition,self.oWmobilePosition,self.oWmobilePosition-self.HoriTopCover-self.HoriSteelDia/2 )
          #有樑，窗，下窗樑,左柱,右柱时：
        """
    ...
    def create_tie_steel_q(self): 
        ...
        steel_list += self.create_tie_steelt(self.WmobilePosition+2*self.VertSteelDia,
                                             self.HoriBottomCover + 3/2*self.HoriSteelDia,
                                             self.Length-self.WmobilePosition-self.WLength_d,
                                             self.WmobilePosition+self.VertSideCover+2*self.VertSteelDia,
                                             self.HoriSteelDia,
                                             self.oWmobilePosition+ self.HoriSteelDia,
                                             0)
            
        steel_list += self.create_tie_steelt(self.WmobilePosition+self.WLength+3/5*self.VertDistance,
                                             self.oWmobilePosition+self.HoriSteelDia+ 1/2*self.VertSteelDia,
                                             self.Length-self.WmobilePosition-self.WLength_d,
                                             self.WmobilePosition+self.WLength+9/10*self.VertDistance,
                                             self.oWmobilePosition - self.VertSteelDia,
                                             self.oWmobilePosition+ self.HoriSteelDia + self.WHeight,
                                             3)
        return steel_list
    def create_tie_steelt(self,start_x,start_xz,end_x,start2_x,start2_z,end_z,flag=0):
        ...
        shape_builder = AllplanReinf.ReinforcementShapeBuilder()
        if self.angle == 90:
            #shape_builder.AddPoints([(AllplanGeo.Point2D(),self.FrontCover-2.6*self.TieSteelDia),
                 #                (AllplanGeo.Point2D(self.Width-2*self.FrontCover, 0), 0),
                   #              (self.FrontCover-3/2*self.TieSteelDia)])
            shape_builder.AddPoints([(AllplanGeo.Point2D(),self.FrontCover-2.2*self.TieSteelDia),
                                 (AllplanGeo.Point2D(self.Width, 0), 0),
                                 (self.FrontCover+2.2*self.TieSteelDia)])
        if self.angle == 135:
            shape_builder.AddPoints([(AllplanGeo.Point2D(),self.FrontCover),
                                 (AllplanGeo.Point2D(self.Width, 0), 0),
                                 (self.FrontCover)])
        shape_builder.SetHookStart(s1,self.angle, AllplanReinf.HookType.eStirrup)
        shape_builder.SetHookEnd(s1,self.angle, AllplanReinf.HookType.eStirrup)

        tie_shape = shape_builder.CreateShape(shape_prop)
        rotation_angles = RotationAngles(0, 0 , 90)
        tie_shape.Rotate(rotation_angles)
        ###
        ...
        if(flag ==3):
            lines = int((end_z -self.oWmobilePosition- self.HoriTopCover - self.HoriBottomCover - self.HoriSteelDia) / self.HoriDistance)

    #垂直放的钢筋数减1      #长度       垂直左或右保护层         垂直钢筋直径              垂直间距
        #rows = int((end_x - 2 * self.VertSideCover - self.VertSteelDia) / self.VertDistance)
        #if(flag == 3):
        rows = int((end_x - start_x - 2 * self.VertSideCover - self.VertSteelDia) / self.VertDistance)
    ...
    def create_add_vertical_steelt(self):
        ...
        for  i in range(int(self.count)):
                ...
                s_shape_12 = AllplanReinf.BendingShape(steel_shape)
                s_shape_12.Move(AllplanGeo.Vector3D(AllplanGeo.Point3D(self.Length-self.WLength_d-self.WmobilePosition-math.sqrt(((i*self.mobile_Length)**2)/2),self.HoriFrontCover+self.VertSteelDia+self.HoriSteelDia+self.TieSteelDia/2,self.oWmobilePosition+self.WHeight+math.sqrt(((i*self.mobile_Length)**2)/2))))
                steel_list.append(AllplanReinf.BarPlacement(0,1,vec,point_f,point_t,s_shape_12))
        
                s_shape_22 = AllplanReinf.BendingShape(steel_shape)
                s_shape_22.Move(AllplanGeo.Vector3D(AllplanGeo.Point3D(self.Length-self.WLength_d-self.WmobilePosition-math.sqrt(((i*self.mobile_Length)**2)/2),self.Width-(self.HoriFrontCover+self.VertSteelDia+self.HoriSteelDia+self.TieSteelDia/2),self.oWmobilePosition+self.WHeight+math.sqrt(((i*self.mobile_Length)**2)/2))))
                steel_list.append(AllplanReinf.BarPlacement(0,1,vec,point_f,point_t,s_shape_22))
        ...
        for  i in range(int(self.count)):
                ...
                #门
                s_shape_52 = AllplanReinf.BendingShape(steel_shape)
                s_shape_52.Move(AllplanGeo.Vector3D(AllplanGeo.Point3D(self.Length-self.WmobilePosition+math.sqrt(((i*self.mobile_Length)**2)/2),self.HoriFrontCover+self.VertSteelDia+self.HoriSteelDia+self.TieSteelDia/2, \
                                                                      self.oWmobilePosition+self.WHeight+math.sqrt(((i*self.mobile_Length)**2)/2))))
                steel_list.append(AllplanReinf.BarPlacement(0,1,vec,point_f,point_t,s_shape_52))

                s_shape_62 = AllplanReinf.BendingShape(steel_shape)
                s_shape_62.Move(AllplanGeo.Vector3D(AllplanGeo.Point3D(self.Length-self.WmobilePosition+math.sqrt(((i*self.mobile_Length)**2)/2),self.Width-(self.HoriFrontCover+self.VertSteelDia+self.HoriSteelDia+self.TieSteelDia/2),  \
                                                                      self.oWmobilePosition+self.WHeight+math.sqrt(((i*self.mobile_Length)**2)/2))))
                steel_list.append(AllplanReinf.BarPlacement(0,1,vec,point_f,point_t,s_shape_62))
    ...
    def create_stirrup(self):
        ...
        if shape_stirrup:
                if not self.DenStir:  #非全程加密
                    p_f_1 = AllplanGeo.Point3D(self.StirDiameter/2,0,self.oWmobilePosition+self.WHeight)
                    p_t_1 = AllplanGeo.Point3D(den_area+self.StirDiameter/2+(self.StirDiameter+self.TieBarDia)*1/2,0,self.oWmobilePosition+self.WHeight)
                ...
                point_cnt_3 = AllplanGeo.Point3D(self.HeadCover,0,self.oWmobilePosition+self.WHeight)
                point_cnt_4 = AllplanGeo.Point3D(self.Length-self.EndCover,self.Width,self.oWmobilePosition+self.WHeight)
    ...
    def create_tie_steel_l(self):
        ...
        if tie_shape:
                if not self.DenStir:
                    distance = 0
                    for x in range(self.WaistNum):
                        p_f_1 = AllplanGeo.Point3D(self.StirDiameter/2+(self.StirDiameter+self.TieBarDia)*3/5,0,self.WaistPosition + 2*self.TieBarDia + distance+self.oWmobilePosition+self.WHeight)
                        p_t_1 = AllplanGeo.Point3D(den_area +self.StirDiameter/2+(self.StirDiameter+self.TieBarDia)*3/5,0,self.WaistPosition + 2*self.TieBarDia + distance+self.oWmobilePosition+self.WHeight)
                        ...
                        p_t_3 = AllplanGeo.Point3D(self.Length -self.EndCover +self.StirDiameter,0,self.WaistPosition + 2*self.TieBarDia + distance+self.oWmobilePosition+self.WHeight)  
                    ...
                    for x in range(self.WaistNum):
                        ...
                        p_t = AllplanGeo.Point3D(self.Length - self.EndCover+self.StirDiameter/2,0,self.WaistPosition + 2*self.TieBarDia + distance+self.oWmobilePosition+self.WHeight)
                        steel_t_1 = LinearBarBuilder.create_linear_bar_placement_from_to_by_dist(0,
                                                                                                    tie_shape,
                                                                                                    p_f,
                                                                                                    p_t,
    ...
    def create_stirrup_z(self):
        ...
        sleeve_cover = ConcreteCoverProperties(self.StirSideCover-self.SleeveThick+self.StirDiameter/2,self.StirSideCover-self.SleeveThick+self.StirDiameter/2,
                                            self.StirFrontCover-self.SleeveThick+self.StirDiameter,self.StirFrontCover-self.SleeveThick)
        cover = ConcreteCoverProperties(self.StirSideCover+self.StirDiameter/2,self.StirSideCover+self.StirDiameter/2,
                                        self.StirFrontCover+self.StirDiameter,self.StirFrontCover)
        ...
        sleeve_stir_shape = GeneralShapeBuilder.create_stirrup_with_user_hooks(self.WmobilePosition +self.rLength,self.Width+self.StirDiameter,
                                                   rotation_angles,
                                                   shape_props,
                                                   sleeve_cover,
                                                   stirrup_type,
                                                   ...
                                                   s1,
                                                   self.Hook_Angle)
        stir_shape = GeneralShapeBuilder.create_stirrup_with_user_hooks(self.WmobilePosition +self.rLength,self.Width+self.StirDiameter,
                                                   rotation_angles,
                                                   shape_props,
                                                   cover,
                                                   stirrup_type,
        ...
        point_f_1 = AllplanGeo.Point3D(-self.rLength,0,0)
        point_t_1 = AllplanGeo.Point3D(-self.rLength,0,self.SleeveAreaLength)
        ...
        point_f_2 = AllplanGeo.Point3D(-self.rLength,0,self.SleeveAreaLength)
        point_t_2 = AllplanGeo.Point3D(-self.rLength,0,self.DensAreaLength)
        stirrup_list.append(LinearBarBuilder.create_linear_bar_placement_from_to_by_dist(0,
                                                                          stir_shape,
                                                                          point_f_2,
                                                                          point_t_2,
        ...
        point_f = AllplanGeo.Point3D(-self.rLength,0,self.DensAreaLength)
        point_t = AllplanGeo.Point3D(-self.rLength,0,self.Height-self.TopDensAreaLength)
        stirrup_list.append(LinearBarBuilder.create_linear_bar_placement_from_to_by_dist(0,
                                                                          stir_shape,
                                                                          point_f,
                                                                          point_t,
        ...
        point_f_3 = AllplanGeo.Point3D(-self.rLength,0,self.Height-self.TopDensAreaLength-self.StirDiameter)
        point_t_3 = AllplanGeo.Point3D(-self.rLength,0,self.Height-self.StirDiameter)
        stirrup_list.append(LinearBarBuilder.create_linear_bar_placement_from_to_by_dist(0,
                                                                          stir_shape,
                                                                          point_f_3,
                                                                          point_t_3,
        ...
        point_f_1 = AllplanGeo.Point3D(self.Length-self.WmobilePosition,0,0)
        point_t_1 = AllplanGeo.Point3D(self.Length-self.WmobilePosition,0,self.SleeveAreaLength)
        stirrup_list.append(LinearBarBuilder.create_linear_bar_placement_from_to_by_dist(0,
                                                                          sleeve_stir_shape,
                                                                          point_f_1,
                                                                          point_t_1,
        ...
        point_f_2 = AllplanGeo.Point3D(self.Length-self.WmobilePosition,0,self.SleeveAreaLength)
        point_t_2 = AllplanGeo.Point3D(self.Length-self.WmobilePosition,0,self.DensAreaLength)
        stirrup_list.append(LinearBarBuilder.create_linear_bar_placement_from_to_by_dist(0,
                                                                          stir_shape,
                                                                          point_f_2,
                                                                          point_t_2,
        ...
        point_f = AllplanGeo.Point3D(self.Length-self.WmobilePosition,0,self.DensAreaLength)
        point_t = AllplanGeo.Point3D(self.Length-self.WmobilePosition,0,self.Height-self.TopDensAreaLength)
        stirrup_list.append(LinearBarBuilder.create_linear_bar_placement_from_to_by_dist(0,
                                                                          stir_shape,
                                                                          point_f,
                                                                          point_t,
        ...
        point_f_3 = AllplanGeo.Point3D(self.Length-self.WmobilePosition,0,self.Height-self.TopDensAreaLength-self.StirDiameter)
        point_t_3 = AllplanGeo.Point3D(self.Length-self.WmobilePosition,0,self.Height-self.StirDiameter)
        stirrup_list.append(LinearBarBuilder.create_linear_bar_placement_from_to_by_dist(0,
                                                                          stir_shape,
                                                                          point_f_3,
                                                                          point_t_3,
    ...
    def create_longitudinal_steel_z(self):
        ...
        point_f_1 = AllplanGeo.Point3D(math.sqrt(((math.sqrt(2.0*((self.StirSideCover+3.0*self.StirDiameter)**2.0))-3.0/2*self.StirDiameter+self.FootSteelDia/2.0)**2)/2),
                                    ...
                                    self.StirFrontCover+self.StirDiameter+self.FootSteelDia/2,
                                    self.VertTopLength_d)
        ...
        point_f_2 = AllplanGeo.Point3D(math.sqrt(((math.sqrt(2.0*((self.StirSideCover+3.0*self.StirDiameter)**2.0))-3.0/2*self.StirDiameter+self.FootSteelDia/2.0)**2)/2),
                                        #self.Width-(math.sqrt(((math.sqrt(2.0*((self.StirFrontCover+3.0*self.StirDiameter)**2.0))-3.0/2*self.StirDiameter+self.FootSteelDia/2.0)**2)/2))-self.FootSteelDia/2,
                                        self.Width - (self.StirFrontCover+self.StirDiameter+self.FootSteelDia/2),
                                        self.VertTopLength_d)
        ...
        #右边的柱子钢筋
         #foot steel
        point_f_1 = AllplanGeo.Point3D(self.Length-self.WmobilePosition+math.sqrt(((math.sqrt(2.0*((self.StirSideCover+3.0*self.StirDiameter)**2.0))-3.0/2*self.StirDiameter+self.FootSteelDia/2.0)**2)/2),
                                    #math.sqrt(((math.sqrt(2.0*((self.StirFrontCover+3.0*self.StirDiameter)**2.0))-3.0/2*self.StirDiameter+self.FootSteelDia/2.0)**2)/2)+self.FootSteelDia/2,
                                    self.StirFrontCover+self.StirDiameter+self.FootSteelDia/2,
                                    self.VertTopLength_d)
        ...
        steel_list.append(LinearBarBuilder.create_linear_bar_placement_from_to_by_count(0,foot_steel_shape,
                                                                                        ...
                                                                                        2))
        #foot steel
        point_f_2 = AllplanGeo.Point3D(self.Length-self.WmobilePosition+math.sqrt(((math.sqrt(2.0*((self.StirSideCover+3.0*self.StirDiameter)**2.0))-3.0/2*self.StirDiameter+self.FootSteelDia/2.0)**2)/2),
                                        #self.Width-(math.sqrt(((math.sqrt(2.0*((self.StirFrontCover+3.0*self.StirDiameter)**2.0))-3.0/2*self.StirDiameter+self.FootSteelDia/2.0)**2)/2))-self.FootSteelDia/2,
                                        self.Width - (self.StirFrontCover+self.StirDiameter+self.FootSteelDia/2),
                                        self.VertTopLength_d)
        ...
        point_f_v1 = AllplanGeo.Point3D(self.Length-self.WmobilePosition+self.StirSideCover+self.StirDiameter+self.VertSteelDia/2,
                                        math.sqrt(((math.sqrt(2.0*((self.StirFrontCover+3.0*self.StirDiameter)**2.0))-3.0/2*self.StirDiameter+self.FootSteelDia/2.0)**2)/2)+self.VertDistance,
                                        self.VertTopLength_d)
        point_t_v1 = AllplanGeo.Point3D(self.Length-self.WmobilePosition+self.StirSideCover+self.StirDiameter+self.VertSteelDia/2,
                                        self.Width-(math.sqrt(((math.sqrt(2.0*((self.StirFrontCover+3.0*self.StirDiameter)**2.0))-3.0/2*self.StirDiameter+self.FootSteelDia/2.0)**2)/2))-self.VertDistance,
                                        self.VertTopLength_d)
        """
        steel_list.append(LinearBarBuilder.create_linear_bar_placement_from_to_by_count(0,steel_shape,
        ...
        steel_list.append(LinearBarBuilder.create_linear_bar_placement_from_to_by_count(0,steel_shape,
                                                                                        ...
                                                                                        self.VertNum))        
        """
        point_f_h1 = AllplanGeo.Point3D(self.Length-self.WmobilePosition+math.sqrt(((math.sqrt(2.0*((self.StirSideCover+3.0*self.StirDiameter)**2.0))-3.0/2*self.StirDiameter+self.FootSteelDia/2.0)**2)/2)+self.HoriDistance_z,
                                        self.StirFrontCover+self.StirDiameter+self.VertSteelDia/2,
                                        self.VertTopLength_d)
        ...
        point_f_h2 = AllplanGeo.Point3D(self.Length-self.WmobilePosition+math.sqrt(((math.sqrt(2.0*((self.StirSideCover+3.0*self.StirDiameter)**2.0))-3.0/2*self.StirDiameter+self.FootSteelDia/2.0)**2)/2)+self.HoriDistance_z,
                                        self.Width-(self.StirFrontCover+self.StirDiameter+self.VertSteelDia/2),
                                        self.VertTopLength_d)
    ...
    def create_tie_steel_z(self):
        ...
        sleeve_tie_shape = sleeve_tie.shape_tie_steel(length=self.Width,width=4*self.TieSteelDia+self.SleeveThick,
                                        ...
                                        degrees=self.TieSideHookAngle-89)
        ...
        #cover3= ConcreteCoverProperties.left_right_bottom(self.StirFrontCover-self.SleeveThick+self.StirDiameter/2,self.StirFrontCover-self.SleeveThick+self.StirDiameter/2,0,0)   
        #cover_3 = ConcreteCoverProperties.left_right_bottom(self.StirFrontCover,self.StirFrontCover,0,0)
        ...
        sleeve_tie2.AddPoints([(AllplanGeo.Point2D(),self.StirFrontCover),
                                 (AllplanGeo.Point2D(self.Width, 0), 0),
                                 (self.StirFrontCover)])
        ...
        ######右边柱子拉筋##############
        vt_sleeve_tie2 = AllplanReinf.ReinforcementShapeBuilder()
        vt_tie2 = AllplanReinf.ReinforcementShapeBuilder()

        vt_sleeve_tie2.AddPoints([(AllplanGeo.Point2D(),self.StirSideCover-self.TieSteelDia),
                                 (AllplanGeo.Point2D(self.WmobilePosition, 0), 0),
                                 (self.StirSideCover-self.TieSteelDia)])
        vt_tie2.AddPoints([(AllplanGeo.Point2D(),self.StirSideCover+self.TieSteelDia),
                                 (AllplanGeo.Point2D(self.WmobilePosition-self.TieSteelDia,0), 0),
                                 (self.StirSideCover)])

        vt_sleeve_tie2.SetHookStart(s1,self.TieSideHookAngle, AllplanReinf.HookType.eStirrup)
        vt_sleeve_tie2.SetHookEnd(s1,self.TieSideHookAngle, AllplanReinf.HookType.eStirrup)

        vt_tie2.SetHookStart(s1,self.TieSideHookAngle, AllplanReinf.HookType.eStirrup)
        vt_tie2.SetHookEnd(s1,self.TieSideHookAngle, AllplanReinf.HookType.eStirrup)

        vt_sleeve_tie2_shape = sleeve_tie2.CreateShape(shape_props)
        angle = RotationAngles(180,0,0) #建立角度旋转偏移的变量   参数： #向高轴偏移的角度,向y轴偏移的角度,向X轴偏移的角度
        vt_sleeve_tie2_shape.Rotate(angle)   #旋转钢筋

        vt_tie2_shape = tie2.CreateShape(shape_props)
        angle = RotationAngles(180,0,0) #建立角度旋转偏移的变量   参数： #向高轴偏移的角度,向y轴偏移的角度,向X轴偏移的角度
        vt_tie2_shape.Rotate(angle)   #旋转钢筋
        ...
        distance = 0
        for x in range(self.HoriNum):
            point_f_1 = AllplanGeo.Point3D(self.Length-self.WmobilePosition+math.sqrt(((math.sqrt(2.0*((self.StirSideCover+3.0*self.StirDiameter)**2.0))-3.0/2*self.StirDiameter+self.FootSteelDia/2.0)**2)/2)+self.HoriDistance_z+distance-self.StirDiameter/2-self.HoriSteelDia/2-self.SleeveThick,
                                            0,
                                            self.StirDiameter)
            point_t_1 = AllplanGeo.Point3D(self.Length-self.WmobilePosition+math.sqrt(((math.sqrt(2.0*((self.StirSideCover+3.0*self.StirDiameter)**2.0))-3.0/2*self.StirDiameter+self.FootSteelDia/2.0)**2)/2)+self.HoriDistance_z+distance-self.StirDiameter/2-self.HoriSteelDia/2-self.SleeveThick,
                                            0,
                                            self.SleeveAreaLength+self.StirDiameter)

            point_f_2 = AllplanGeo.Point3D(self.Length-self.WmobilePosition+math.sqrt(((math.sqrt(2.0*((self.StirSideCover+3.0*self.StirDiameter)**2.0))-3.0/2*self.StirDiameter+self.FootSteelDia/2.0)**2)/2)+self.HoriDistance_z+distance-self.StirDiameter/2-self.HoriSteelDia/2,
                                            0,
                                            self.SleeveAreaLength+self.StirDiameter)
            point_t_2 = AllplanGeo.Point3D(self.Length-self.WmobilePosition+math.sqrt(((math.sqrt(2.0*((self.StirSideCover+3.0*self.StirDiameter)**2.0))-3.0/2*self.StirDiameter+self.FootSteelDia/2.0)**2)/2)+self.HoriDistance_z+distance-self.StirDiameter/2-self.HoriSteelDia/2,
                                            0,
                                            self.DensAreaLength+self.StirDiameter)

            point_f = AllplanGeo.Point3D(self.Length-self.WmobilePosition+math.sqrt(((math.sqrt(2.0*((self.StirSideCover+3.0*self.StirDiameter)**2.0))-3.0/2*self.StirDiameter+self.FootSteelDia/2.0)**2)/2)+self.HoriDistance_z+distance-self.StirDiameter/2-self.HoriSteelDia/2,
                                            0,
                                            self.DensAreaLength+self.StirDiameter)
            point_t = AllplanGeo.Point3D(self.Length-self.WmobilePosition+math.sqrt(((math.sqrt(2.0*((self.StirSideCover+3.0*self.StirDiameter)**2.0))-3.0/2*self.StirDiameter+self.FootSteelDia/2.0)**2)/2)+self.HoriDistance_z+distance-self.StirDiameter/2-self.HoriSteelDia/2,
                                            0,
                                            self.Height+self.StirDiameter-self.TopDensAreaLength)

            point_f_3 = AllplanGeo.Point3D(self.Length-self.WmobilePosition+math.sqrt(((math.sqrt(2.0*((self.StirSideCover+3.0*self.StirDiameter)**2.0))-3.0/2*self.StirDiameter+self.FootSteelDia/2.0)**2)/2)+self.HoriDistance_z+distance-self.StirDiameter/2-self.HoriSteelDia/2,
                                            0,
                                            self.Height-self.TopDensAreaLength)
            point_t_3 = AllplanGeo.Point3D(self.Length-self.WmobilePosition+math.sqrt(((math.sqrt(2.0*((self.StirSideCover+3.0*self.StirDiameter)**2.0))-3.0/2*self.StirDiameter+self.FootSteelDia/2.0)**2)/2)+self.HoriDistance_z+distance-self.StirDiameter/2-self.HoriSteelDia/2,
                                            0,
                                            self.Height)
            ...
            if x != self.HoriNum - 1:
                ...
                if x % 2 == 0:
                    t_point_f_1 = AllplanGeo.Point3D(self.Length-self.WmobilePosition+math.sqrt(((math.sqrt(2.0*((self.StirSideCover+3.0*self.StirDiameter)**2.0))-3.0/2*self.StirDiameter+self.FootSteelDia/2.0)**2)/2)+self.HoriDistance_z+distance+self.StirDiameter/2+self.HoriSteelDia/2+self.SleeveThick,
                                            0,
                                            self.StirDiameter)
                    t_point_t_1 = AllplanGeo.Point3D(self.Length-self.WmobilePosition+math.sqrt(((math.sqrt(2.0*((self.StirSideCover+3.0*self.StirDiameter)**2.0))-3.0/2*self.StirDiameter+self.FootSteelDia/2.0)**2)/2)+self.HoriDistance_z+distance+self.StirDiameter/2+self.HoriSteelDia/2+self.SleeveThick,
                                                    0,
                                                    self.SleeveAreaLength+self.StirDiameter)

                    t_point_f_2 = AllplanGeo.Point3D(self.Length-self.WmobilePosition+math.sqrt(((math.sqrt(2.0*((self.StirSideCover+3.0*self.StirDiameter)**2.0))-3.0/2*self.StirDiameter+self.FootSteelDia/2.0)**2)/2)+self.HoriDistance_z+distance+self.StirDiameter/2+self.HoriSteelDia/2,
                                                    0,
                                                    self.SleeveAreaLength+self.StirDiameter)
                    t_point_t_2 = AllplanGeo.Point3D(self.Length-self.WmobilePosition+math.sqrt(((math.sqrt(2.0*((self.StirSideCover+3.0*self.StirDiameter)**2.0))-3.0/2*self.StirDiameter+self.FootSteelDia/2.0)**2)/2)+self.HoriDistance_z+distance+self.StirDiameter/2+self.HoriSteelDia/2,
                                                    0,
                                                    self.DensAreaLength+self.StirDiameter)

                    t_point_f = AllplanGeo.Point3D(self.Length-self.WmobilePosition+math.sqrt(((math.sqrt(2.0*((self.StirSideCover+3.0*self.StirDiameter)**2.0))-3.0/2*self.StirDiameter+self.FootSteelDia/2.0)**2)/2)+self.HoriDistance_z+distance+self.StirDiameter/2+self.HoriSteelDia/2,
                                                    0,
                                                    self.DensAreaLength+self.StirDiameter)
                    t_point_t = AllplanGeo.Point3D(self.Length-self.WmobilePosition+math.sqrt(((math.sqrt(2.0*((self.StirSideCover+3.0*self.StirDiameter)**2.0))-3.0/2*self.StirDiameter+self.FootSteelDia/2.0)**2)/2)+self.HoriDistance_z+distance+self.StirDiameter/2+self.HoriSteelDia/2,
                                                    0,
                                                    self.Height+self.StirDiameter-self.TopDensAreaLength)

                    t_point_f_3 = AllplanGeo.Point3D(self.Length-self.WmobilePosition+math.sqrt(((math.sqrt(2.0*((self.StirSideCover+3.0*self.StirDiameter)**2.0))-3.0/2*self.StirDiameter+self.FootSteelDia/2.0)**2)/2)+self.HoriDistance_z+distance+self.StirDiameter/2+self.HoriSteelDia/2,
                                                    0,
                                                    self.Height-self.TopDensAreaLength)
                    t_point_t_3 = AllplanGeo.Point3D(self.Length-self.WmobilePosition+math.sqrt(((math.sqrt(2.0*((self.StirSideCover+3.0*self.StirDiameter)**2.0))-3.0/2*self.StirDiameter+self.FootSteelDia/2.0)**2)/2)+self.HoriDistance_z+distance+self.StirDiameter/2+self.HoriSteelDia/2,
                                                    0,
                                                    self.Height)                    
        ...
        return steel_list
    def create_vertical_steel_d(self):
        steel_list = []
        ...
        if self.BendingAnchor: #弯曲锚   #锚
            steel_vert_1 = longit.shape_anchor_steel(self.WHeight +self.oWmobilePosition,self.Length_TA,self.BendPosition,self.BendLength,self.BendWidth,2,False,True)
            steel_vert_2 = longit.shape_anchor_steel(self.WHeight +self.oWmobilePosition,self.Length_TA,self.BendPosition,self.BendLength,self.BendWidth,2,True,True)

        else:
            steel_vert_1 = steel_vert_2 = longit.shape_extend_steel(length=self.WHeight +self.oWmobilePosition,extend=self.Length_TA,extend_side=2,vertical=True)

        point_f_1 = AllplanGeo.Point3D(self.WLength+self.WmobilePosition+1/2*self.VertDistance,self.HoriFrontCover + self.HoriSteelDia,0)
        point_t_1 = AllplanGeo.Point3D(self.Length-self.WLength_d-self.WmobilePosition+self.Hole_cover*2,self.HoriFrontCover + self.HoriSteelDia,0)
        ...
        point_f_2 = AllplanGeo.Point3D(self.WLength+self.WmobilePosition+1/2*self.VertDistance,self.Width - self.HoriFrontCover - self.HoriSteelDia,0)
        point_t_2 = AllplanGeo.Point3D(self.Length-self.WLength_d-self.WmobilePosition+self.Hole_cover*2,self.Width - self.HoriFrontCover - self.HoriSteelDia,0)
    ...
    def create_horizontal_steelt_d(self):  
        steel_list = []
                    #保护层混凝土属性
        #cover = ConcreteCoverProperties.left_right(self.HoriSideCover,self.HoriSideCover)
        cover = ConcreteCoverProperties(0,0,0,self.HoriBottomCover) #self.VertBottomCover)#第四个参数是设置钢筋元素离底部的距离多少开始
        ...
        steel_shape = longit.shape_steel(self.Length - self.WLength-self.WLength_d - self.WmobilePosition*2 -self.Hole_cover*2)

        point_1 = AllplanGeo.Point3D(self.HoriFrontCover+self.WmobilePosition+self.WLength,self.FrontCover+self.HoriSteelDia/2,self.oWmobilePosition)
        point_2 = AllplanGeo.Point3D(self.HoriFrontCover+self.WmobilePosition+self.WLength,self.FrontCover+self.HoriSteelDia/2,self.WHeight+self.oWmobilePosition)
        point_3 = AllplanGeo.Point3D(self.HoriFrontCover+self.WmobilePosition+self.WLength,self.Width-self.FrontCover-self.HoriSteelDia/2,self.oWmobilePosition)
        point_4 = AllplanGeo.Point3D(self.HoriFrontCover+self.WmobilePosition+self.WLength,self.Width-self.FrontCover-self.HoriSteelDia/2,self.WHeight+self.oWmobilePosition)
        steel_list.append(LinearBarBuilder.create_linear_bar_placement_from_to_by_dist(0,
                                                                                        steel_shape,
                                                                                        point_1,
                                                                                        point_2,
