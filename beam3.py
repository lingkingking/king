...
class Beam(object):
    ...
    def shape_tie_steel(self,length,width):
        ...
        if self.Tie_Hook_Angle == 90:
            hook_angle=self.Tie_Hook_Angle - 89
        if self.Tie_Hook_Angle == 135:
            hook_angle=self.Tie_Hook_Angle -180

        shape = sleeve_tie.shape_tie_steel(length=length,width=width,
                                        rotate=RotationAngles(90,0,90),hook=s1-20,
                                        degrees=hook_angle)
    ...
    def create_stirrup(self):
        ...
        shape = GeneralShapeBuilder.create_stirrup_with_user_hooks(self.Height+self.StirExtendLength,self.Width,
                                                   rotation_angles,
                                                   shape_props,
                                                   concrete_props,
                                                   stirrup_type,
