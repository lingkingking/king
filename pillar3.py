...
class Pillar(object):
    ...
    def create_stirrup(self):
        ...
        #保护层混凝土属性
        sleeve_cover = ConcreteCoverProperties(self.StirSideCover-self.SleeveThick,self.StirSideCover-self.SleeveThick,
                                            self.StirFrontCover-self.SleeveThick,self.StirFrontCover-self.SleeveThick)
        cover = ConcreteCoverProperties(self.StirSideCover,self.StirSideCover,
                                        self.StirFrontCover,self.StirFrontCover)
        ...
        if self.StirExtend:
            ...
            #elif self.Hook_Angle ==90:
                #stirrup_type =AllplanReinf.StirrupType.Torsion
            rotation_angles     = RotationAngles(0, 0 ,0)
            sleeve_stir_shape = GeneralShapeBuilder.create_stirrup_with_user_hooks(self.Length,self.Width,
                                                   rotation_angles,
                                                   shape_props,
                                                   sleeve_cover,
                                                   stirrup_type,
                                                   ...
                                                   s1,
                                                   self.Hook_Angle)
            stir_shape = GeneralShapeBuilder.create_stirrup_with_user_hooks(self.Length,self.Width,
                                                   rotation_angles,
                                                   shape_props,
                                                   cover,
                                                   stirrup_type,
        ...
        if(self.Length > self.Width):
            den_area = max(self.Length,self.Height/6,500)
        else:
            den_area = max(self.Width,self.Height/6,500)
        ...
        point_t_2 = AllplanGeo.Point3D(0,0,den_area)
        stirrup_list.append(LinearBarBuilder.create_linear_bar_placement_from_to_by_dist(0,
                                                                          stir_shape,
                                                                          point_f_2,
                                                                          point_t_2,
        ...
        point_f = AllplanGeo.Point3D(0,0,den_area+self.StirDiameter)
        point_t = AllplanGeo.Point3D(0,0,self.Height-den_area+self.StirDiameter)
        stirrup_list.append(LinearBarBuilder.create_linear_bar_placement_from_to_by_dist(0,
                                                                          stir_shape,
                                                                          point_f,
                                                                          point_t,
        ...
        point_f_3 = AllplanGeo.Point3D(0,0,self.Height-den_area)
    ...
    def create_longitudinal_steel(self):
        ...
        point_list.append((AllplanGeo.Point3D(self.Height,0,0),0))
        point_list.append((AllplanGeo.Point3D(self.Height+self.VertExtendLength,0,0),0))
        point_list.append((AllplanGeo.Point3D(self.Height+self.VertExtendLength,self.Vert_steel_wanqu,0),0))
        ...
        steel_shape = shape_build.CreateShape(shape_props)
        angle = RotationAngles(-90,-90,0)
        steel_shape.Rotate(angle)
        shape_props = ReinforcementShapeProperties.rebar(**rebar_prop)
        steel_shape2 = shape_build.CreateShape(shape_props)
        angle = RotationAngles(90,-90,0)
        steel_shape2.Rotate(angle)

        shape_props = ReinforcementShapeProperties.rebar(**hori_rebar_prop)
        hori_steel_shape = shape_build.CreateShape(shape_props)
        angle = RotationAngles(0,-90,0)
        hori_steel_shape.Rotate(angle)
        shape_props = ReinforcementShapeProperties.rebar(**hori_rebar_prop)
        hori_steel_shape2 = shape_build.CreateShape(shape_props)
        angle = RotationAngles(180,-90,0)
        hori_steel_shape2.Rotate(angle)

        shape_props = ReinforcementShapeProperties.rebar(**foot_rebar_prop)
        foot_steel_shape = shape_build.CreateShape(shape_props)
        angle = RotationAngles(0,-90,0)
        foot_steel_shape.Rotate(angle)

        shape_props = ReinforcementShapeProperties.rebar(**foot_rebar_prop)
        foot_steel_shape2 = shape_build.CreateShape(shape_props)
        angle = RotationAngles(180,-90,0)
        foot_steel_shape2.Rotate(angle)
        ###############
        ...
        #foot steel
        #point_f_1 = AllplanGeo.Point3D(math.sqrt(((math.sqrt(2.0*((self.StirSideCover+3.0*self.StirDiameter)**2.0))-3.0/2*self.StirDiameter+self.FootSteelDia/2.0)**2)/2 - math.sqrt((self.FootSteelDia**2)/2)),
                                 #   math.sqrt(((math.sqrt(2.0*((self.StirFrontCover+3.0*self.StirDiameter)**2.0))-3.0/2*self.StirDiameter+self.FootSteelDia/2.0)**2)/2)+self.FootSteelDia/2 - math.sqrt((self.FootSteelDia**2)/2),
                               #     self.Vert_steel_cover)
        #point_t_1 = AllplanGeo.Point3D(self.Length-math.sqrt(((math.sqrt(2.0*((self.StirSideCover+3.0*self.StirDiameter)**2.0))-3.0/2*self.StirDiameter+self.FootSteelDia/2.0)**2)/2 - math.sqrt((self.FootSteelDia**2)/2)),
                             #       math.sqrt(((math.sqrt(2.0*((self.StirFrontCover+3.0*self.StirDiameter)**2.0))-3.0/2*self.StirDiameter+self.FootSteelDia/2.0)**2)/2)+self.FootSteelDia/2 - math.sqrt((self.FootSteelDia**2)/2),
                             #       self.Vert_steel_cover)

        point_f_1 = AllplanGeo.Point3D(self.StirSideCover+3.0*self.StirDiameter-self.StirDiameter*math.sqrt(2)+self.VertSteelDia/math.sqrt(8)-self.FootSteelDia*3.85/10,
                                       self.StirFrontCover+3.0*self.StirDiameter-self.StirDiameter*math.sqrt(2)+self.VertSteelDia/math.sqrt(8),
                                       self.Vert_steel_cover)
        point_t_1 = AllplanGeo.Point3D(self.Length-(self.StirSideCover+3.0*self.StirDiameter-self.StirDiameter*math.sqrt(2)+self.VertSteelDia/math.sqrt(8) -self.FootSteelDia*3.85/10),
                                    self.StirFrontCover+3.0*self.StirDiameter-self.StirDiameter*math.sqrt(2)+self.VertSteelDia/math.sqrt(8)
                                    ,
                                    self.Vert_steel_cover)
        ...
        point_f_2 = AllplanGeo.Point3D(self.StirSideCover+3.0*self.StirDiameter-self.StirDiameter*math.sqrt(2)+self.VertSteelDia/math.sqrt(8) -self.FootSteelDia*3.85/10,
                                       self.Width-(self.StirFrontCover+3.0*self.StirDiameter-self.StirDiameter*math.sqrt(2)+self.VertSteelDia/math.sqrt(8)),self.Vert_steel_cover)
        point_t_2 = AllplanGeo.Point3D(self.Length-(self.StirSideCover+3.0*self.StirDiameter-self.StirDiameter*math.sqrt(2)+self.VertSteelDia/math.sqrt(8) -self.FootSteelDia*3.85/10),
                                        self.Width-(self.StirFrontCover+3.0*self.StirDiameter-self.StirDiameter*math.sqrt(2)+self.VertSteelDia/math.sqrt(8)),self.Vert_steel_cover)
        steel_list.append(LinearBarBuilder.create_linear_bar_placement_from_to_by_count(0,foot_steel_shape2,
                                                                                        point_f_2,point_t_2,
                                                                                        0,
                                                                                        0,
                                                                                        2))
        ...
        point_t_v2 = AllplanGeo.Point3D(self.Length-(self.StirSideCover+self.StirDiameter+self.VertSteelDia/2),
                                        self.Width-(math.sqrt(((math.sqrt(2.0*((self.StirFrontCover+3.0*self.StirDiameter)**2.0))-3.0/2*self.StirDiameter+self.FootSteelDia/2.0)**2)/2))-self.VertDistance,
                                        self.Vert_steel_cover)
        steel_list.append(LinearBarBuilder.create_linear_bar_placement_from_to_by_count(0,steel_shape2,
        ...
        point_t_h2 = AllplanGeo.Point3D(self.Length-(math.sqrt(((math.sqrt(2.0*((self.StirSideCover+3.0*self.StirDiameter)**2.0))-3.0/2*self.StirDiameter+self.FootSteelDia/2.0)**2)/2))-self.HoriDistance,
                                        self.Width-(self.StirFrontCover+self.StirDiameter+self.VertSteelDia/2),
                                        self.Vert_steel_cover)
        steel_list.append(LinearBarBuilder.create_linear_bar_placement_from_to_by_count(0,hori_steel_shape2,
    ...
    def create_tie_steel(self):
        ...
        sleeve_tie2.AddPoints([(AllplanGeo.Point2D(),self.StirFrontCover-self.TieSteelDia),
                                 (AllplanGeo.Point2D(self.Width, 0), 0),
                                 (self.StirFrontCover-self.TieSteelDia)])
        ...
        vt_tie2.AddPoints([(AllplanGeo.Point2D(),self.StirSideCover+self.TieSteelDia),
                                 (AllplanGeo.Point2D(self.Length-self.TieSteelDia,0), 0),
                                 (self.StirSideCover)])
        ...
        h_cover = ConcreteCoverProperties(-self.SleeveThick/2,-self.SleeveThick/2,self.StirFrontCover-self.SleeveThick,self.StirFrontCover-self.SleeveThick)        
        h_cover_1 = ConcreteCoverProperties(0,0,self.StirFrontCover,self.StirFrontCover)
        ...
        ###########
        shape_props =ReinforcementShapeProperties.rebar(self.TieSteelDia,self.BendingRoller,self.TieSteelGrade,self.ConcreteGrade,AllplanReinf.BendingShapeType.LongitudinalBar)
        s1=max(10*self.TieSteelDia,75)
        #if self.Hook_Angle == 135:
        stirrup_type = AllplanReinf.StirrupType.Column
        #elif self.Hook_Angle ==90:
               # stirrup_type =AllplanReinf.StirrupType.Torsion
        rotation_angles = RotationAngles(0, 0 ,0)
        dh_sleeve_stirrup_shape = GeneralShapeBuilder.create_stirrup_with_user_hooks(dh+self.HoriSteelDia/2+2*self.StirDiameter,self.Width,
                                                   rotation_angles,
                                                   shape_props,
                                                   h_cover,
                                                   stirrup_type,
                                                   ...
                                                   s1,
                                                   self.Hook_Angle)
        dh_stirrup_shape = GeneralShapeBuilder.create_stirrup_with_user_hooks(dh+self.HoriSteelDia/2+2*self.StirDiameter,self.Width,
                                                   rotation_angles,
                                                   shape_props,
                                                   h_cover_1,
                                                   stirrup_type,
        ...
        v_cover = ConcreteCoverProperties(self.StirSideCover-self.SleeveThick,self.StirSideCover-self.SleeveThick,
                                            -self.SleeveThick/2,-self.SleeveThick/2)        
        v_cover_1 = ConcreteCoverProperties(self.StirSideCover,self.StirSideCover,0,0)
        ...
        rotation_angles     = RotationAngles(0, 0 ,0)
        """
        dv_sleeve_stirrup_shape = GeneralShapeBuilder.create_stirrup(self.Length,dv+self.VertSteelDia/2+2*self.StirDiameter+2*self.SleeveThick,
                                                   rotation_angles,
                                                   shape_props,
                                                   v_cover,
                                                   stirrup_type,
                                                   s1)
        dv_stirrup_shape = GeneralShapeBuilder.create_stirrup(self.Length,dv+self.VertSteelDia/2+2*self.StirDiameter,
                                                   rotation_angles,
                                                   shape_props,
                                                   v_cover_1,
                                                   stirrup_type,
                                                   s1)
        """
        dv_sleeve_stirrup_shape = GeneralShapeBuilder.create_stirrup_with_user_hooks(self.Length,dv+self.VertSteelDia/2+2*self.StirDiameter,
                                                   rotation_angles,
                                                   shape_props,
                                                   v_cover,
                                                   stirrup_type,
                                                   ...
                                                   s1,
                                                   self.Hook_Angle)
        dv_stirrup_shape = GeneralShapeBuilder.create_stirrup_with_user_hooks(self.Length,dv+self.VertSteelDia/2+2*self.StirDiameter,
                                                   rotation_angles,
                                                   shape_props,
                                                   v_cover_1,
                                                   stirrup_type,
        ...
        for x in range(self.HoriNum):
            point_f_1 = AllplanGeo.Point3D(math.sqrt(((math.sqrt(2.0*((self.StirSideCover+3.0*self.StirDiameter)**2.0))-3.0/2*self.StirDiameter+self.FootSteelDia/2.0)**2)/2)+self.HoriDistance+distance-self.StirDiameter/2-self.HoriSteelDia/2,
                                            0,
                                            self.StirDiameter)
            point_t_1 = AllplanGeo.Point3D(math.sqrt(((math.sqrt(2.0*((self.StirSideCover+3.0*self.StirDiameter)**2.0))-3.0/2*self.StirDiameter+self.FootSteelDia/2.0)**2)/2)+self.HoriDistance+distance-self.StirDiameter/2-self.HoriSteelDia/2,
                                            0,
            ...
            point_f_3 = AllplanGeo.Point3D(math.sqrt(((math.sqrt(2.0*((self.StirSideCover+3.0*self.StirDiameter)**2.0))-3.0/2*self.StirDiameter+self.FootSteelDia/2.0)**2)/2)+self.HoriDistance+distance-self.StirDiameter/2-self.HoriSteelDia/2,
                                            0,
                                            self.Height+self.StirDiameter-den_area)
            point_t_3 = AllplanGeo.Point3D(math.sqrt(((math.sqrt(2.0*((self.StirSideCover+3.0*self.StirDiameter)**2.0))-3.0/2*self.StirDiameter+self.FootSteelDia/2.0)**2)/2)+self.HoriDistance+distance-self.StirDiameter/2-self.HoriSteelDia/2,
                                            0,
                                            self.Height+2*self.StirDiameter)
            ...
            if x != self.HoriNum - 1:
                ...
                if x % 2 == 0:
                    t_point_f_1 = AllplanGeo.Point3D(math.sqrt(((math.sqrt(2.0*((self.StirSideCover+3.0*self.StirDiameter)**2.0))-3.0/2*self.StirDiameter+self.FootSteelDia/2.0)**2)/2)+self.HoriDistance+distance+self.StirDiameter/2+self.HoriSteelDia/2,
                                            0,
                                            self.StirDiameter)
                    t_point_t_1 = AllplanGeo.Point3D(math.sqrt(((math.sqrt(2.0*((self.StirSideCover+3.0*self.StirDiameter)**2.0))-3.0/2*self.StirDiameter+self.FootSteelDia/2.0)**2)/2)+self.HoriDistance+distance+self.StirDiameter/2+self.HoriSteelDia/2,
                                                    0,
                                                    self.SleeveAreaLength+self.StirDiameter)
        ...
        for x in range(self.VertNum):
            point_f_1 = AllplanGeo.Point3D(0,
                                            ...
                                            3/2*self.StirDiameter+self.TieSteelDia)
            point_t_1 = AllplanGeo.Point3D(0,
                                            math.sqrt(((math.sqrt(2.0*((self.StirFrontCover+3.0*self.StirDiameter)**2.0))-3.0/2*self.StirDiameter+self.FootSteelDia/2.0)**2)/2)+self.VertDistance+distance-self.StirDiameter/2-self.VertSteelDia/2,
                                            self.SleeveAreaLength+3/2*self.StirDiameter+self.TieSteelDia)
            ...
            point_t_2 = AllplanGeo.Point3D(0,
                                            math.sqrt(((math.sqrt(2.0*((self.StirFrontCover+3.0*self.StirDiameter)**2.0))-3.0/2*self.StirDiameter+self.FootSteelDia/2.0)**2)/2)+self.VertDistance+distance-self.StirDiameter/2-self.VertSteelDia/2,
                                            den_area+3/2*self.StirDiameter+self.TieSteelDia)

            point_f = AllplanGeo.Point3D(0,
                                            math.sqrt(((math.sqrt(2.0*((self.StirFrontCover+3.0*self.StirDiameter)**2.0))-3.0/2*self.StirDiameter+self.FootSteelDia/2.0)**2)/2)+self.VertDistance+distance-self.StirDiameter/2-self.VertSteelDia/2,
                                            den_area+5/2*self.StirDiameter+self.TieSteelDia)
            point_t = AllplanGeo.Point3D(0,
                                            math.sqrt(((math.sqrt(2.0*((self.StirFrontCover+3.0*self.StirDiameter)**2.0))-3.0/2*self.StirDiameter+self.FootSteelDia/2.0)**2)/2)+self.VertDistance+distance-self.StirDiameter/2-self.VertSteelDia/2,
                                            self.Height+self.TieSteelDia-den_area+1/2*self.StirDiameter)

            point_f_3 = AllplanGeo.Point3D(0,
                                            math.sqrt(((math.sqrt(2.0*((self.StirFrontCover+3.0*self.StirDiameter)**2.0))-3.0/2*self.StirDiameter+self.FootSteelDia/2.0)**2)/2)+self.VertDistance+distance-self.StirDiameter/2-self.VertSteelDia/2,
                                            self.Height+3/2*self.StirDiameter+self.TieSteelDia-den_area)
            point_t_3 = AllplanGeo.Point3D(0,
                                            math.sqrt(((math.sqrt(2.0*((self.StirFrontCover+3.0*self.StirDiameter)**2.0))-3.0/2*self.StirDiameter+self.FootSteelDia/2.0)**2)/2)+self.VertDistance+distance-self.StirDiameter/2-self.VertSteelDia/2,
                                            self.Height+5/2*self.StirDiameter+self.TieSteelDia)            
            ...
            if x != self.VertNum - 1:
                ...
                if x % 2 == 0:
                    ...
                    vt_point_t = AllplanGeo.Point3D(0,
                                                    math.sqrt(((math.sqrt(2.0*((self.StirFrontCover+3.0*self.StirDiameter)**2.0))-3.0/2*self.StirDiameter+self.FootSteelDia/2.0)**2)/2)+self.VertDistance+distance+self.StirDiameter/2+self.VertSteelDia/2,
                                                    self.Height+self.TieSteelDia-den_area+self.StirDiameter)
